...
app = dash.Dash(__name__, update_title=None, external_stylesheets=external_stylesheets, meta_tags=[{'name': 'Cache-control', 'content': 'no-cache, no-store, must-revalidate'}, {'name': 'Pragma', 'content': 'no-cache'}, {'name': 'Expires', 'content': '0'}])
...
def toggle_tooltip(is_open, children):
    ...
    if is_open :
        if walletDataCol.count_documents({"$and": [{'wallet': children},{'arena': "League 1"}]}) > 0 :
            registration = 10
            arena = list(walletDataCol.aggregate([{"$match": {"$and": [{'wallet': children},{'arena': "League 1"}]}},{"$group": {"_id": "null","totalAmount": {"$sum": "$xkiSpent"}}}]))
        ...
        if walletDataCol.count_documents({"$and": [{'wallet': children},{'arena': "Training"}]}) > 0 :
            training = list(walletDataCol.aggregate([{"$match": {"$and": [{'wallet': children},{'arena': "Training"}]}},{"$group": {"_id": "null","totalAmount": {"$sum": "$xkiSpent"}}}]))
            training = "{:.5}".format(round(training[0]["totalAmount"] / 1000000,4))

        if walletBoostCol.count_documents({'wallet': children}) > 0 :
            boosts = list(walletBoostCol.aggregate([{"$match": {'wallet': children}},{"$group": {"_id": "null","totalAmount": {"$sum": "$xkiSpent"}}}]))
            boosts = "{:.5}".format(round(boosts[0]["totalAmount"] / 1000000,4))
        
        detailed_fees = "Reward: +" + str(reward) + " xki " + "Registration: -" + str(registration) + " xki " + "Arena fees: -" + str(arena) + " xki " + "Training fees: -" + str(training) + " xki " + "Boosts : -" + str(boosts) + " xki "
...
@app.callback(
    ...
    Output("table-output-leaderboard", "data"), Output('table-output-leaderboard', 'columns'), Output('table-output-leaderboard', 'style_data_conditional'),
...
def update_output(children, date):
    ...
    xki_reward = 0.0
    if len(children) == 0:
        return (data_arena, columns_arena, data_training, columns_training, data_leaderboard, columns_leaderboard, style_data_conditional, number_fights_league, winrate, xki_str, rank)
    if walletListCol.count_documents({'wallet': children}, limit = 1) == 0 :
        walletListCol.insert_one({'wallet' : children})
        return (data_arena, columns_arena, data_training, columns_training, data_leaderboard, columns_leaderboard, style_data_conditional, "Your wallet has been added to our database.", "Press the get data button to show your stats !", xki_str, rank)
    else : 
        date = datetime.strptime(date, '%Y-%m-%dT%H:%M')
        number_fights_league = walletDataCol.count_documents({"$and": [{'wallet': children},{'arena': "League 1"}, {'txDate' : {'$gte' : date}}]})
        if number_fights_league > 0 :
            df = pd.DataFrame(list(walletDataCol.find({"$and": [{'wallet': children},{'arena': "League 1"}, {'txDate' : {'$gte' : date}}]}, {'_id': 0, 'my_deck_id': 1, 'my_deck_score':1, 'my_deck_score_bonus':1,'score_diff':1,'my_cosmon_1_nft_id': 1,'my_cosmon_1_nft_name': 1, 'my_cosmon_1_lvl' :1,'my_cosmon_2_nft_id': 1,'my_cosmon_2_nft_name': 1, 'my_cosmon_2_lvl' :1,'my_cosmon_3_nft_id': 1,'my_cosmon_3_nft_name': 1, 'my_cosmon_3_lvl' :1, 'winner': 1})))
            #get number of fights in league
            df_count = df.groupby(['my_cosmon_1_nft_id','my_cosmon_2_nft_id','my_cosmon_3_nft_id']).size().reset_index(name='Fights')
            #get wins/draws/losses
            df_winner= df[df['winner'] == 2]
            df_winner = df_winner.groupby(['my_cosmon_1_nft_id','my_cosmon_2_nft_id','my_cosmon_3_nft_id']).size().reset_index(name='Won')
            df_draw= df[df['winner'] == 0]
            df_draw = df_draw.groupby(['my_cosmon_1_nft_id','my_cosmon_2_nft_id','my_cosmon_3_nft_id']).size().reset_index(name='Drawn')
            df_lost= df[df['winner'] == -1]
            df_lost = df_lost.groupby(['my_cosmon_1_nft_id','my_cosmon_2_nft_id','my_cosmon_3_nft_id']).size().reset_index(name='Lost')
            #merge to get the winrate
            df_output = df_count.merge(df_winner, on=['my_cosmon_1_nft_id','my_cosmon_2_nft_id','my_cosmon_3_nft_id'], how='outer').fillna(0)
            df_output = df_output.merge(df_draw, on=['my_cosmon_1_nft_id','my_cosmon_2_nft_id','my_cosmon_3_nft_id'], how='outer').fillna(0)
            df_output = df_output.merge(df_lost, on=['my_cosmon_1_nft_id','my_cosmon_2_nft_id','my_cosmon_3_nft_id'], how='outer').fillna(0)
            df_output["Winrate"] = df_output["Won"] / df_output["Fights"]
            #drop duplicates
            df_output = df_output.merge(df.drop_duplicates(['my_cosmon_1_nft_id','my_cosmon_2_nft_id','my_cosmon_3_nft_id'], keep='last'),how='left',on=['my_cosmon_1_nft_id','my_cosmon_2_nft_id','my_cosmon_3_nft_id'])
            #get points scored per deck
            df_output["Points"] = df_output["Won"] *2 + df_output["Lost"] * -1
            #format the dataframe
            df_output = df_output.rename(columns={'my_cosmon_1_nft_name': 'Cosmon #1', 'my_cosmon_2_nft_name': 'Cosmon #2', 'my_cosmon_3_nft_name': 'Cosmon #3', 'my_cosmon_1_lvl': 'Level #1', 'my_cosmon_2_lvl': 'Level #2', 'my_cosmon_3_lvl': 'Level #3'})
            df_output = df_output[['Fights', 'Won', 'Drawn', 'Lost', 'Winrate','Points','Cosmon #1','Level #1','Cosmon #2','Level #2','Cosmon #3','Level #3']]
        ...
        number_fights_training = walletDataCol.count_documents({"$and": [{'wallet': children},{'arena': "Training"}, {'txDate' : {'$gte' : date}}]})
        if number_fights_training > 0 :
            df = pd.DataFrame(list(walletDataCol.find({"$and": [{'wallet': children},{'arena': "Training"}, {'txDate' : {'$gte' : date}}]}, {'_id': 0, 'my_deck_id': 1, 'my_deck_score':1, 'my_deck_score_bonus':1,'score_diff':1, 'my_cosmon_1_nft_id': 1, 'my_cosmon_1_nft_name': 1, 'my_cosmon_1_lvl' :1,'my_cosmon_2_nft_id': 1 ,'my_cosmon_2_nft_name': 1, 'my_cosmon_2_lvl' :1,'my_cosmon_3_nft_id': 1 ,'my_cosmon_3_nft_name': 1, 'my_cosmon_3_lvl' :1, 'winner': 1})))
            #get number of fights in league
            df_count = df.groupby(['my_cosmon_1_nft_id','my_cosmon_2_nft_id','my_cosmon_3_nft_id']).size().reset_index(name='Fights')
             #get wins/draws/losses
            df_winner= df[df['winner'] == 2]
        ...
        #get leaderboard
        df = pd.DataFrame(list(leaderboardCol.find({}, {'_id': 0, 'wallet': 1, 'points':1, 'rank':1, 'reward': 1, 'fights':1, 'won':1, 'drawn': 1, 'lost':1, 'winrate': 1, 'boostNumber':1})))
        df_output = df.rename(columns={'wallet': 'Wallet', 'points': 'Score', 'rank': 'Rank', 'reward': 'Reward ($XKI)', 'fights': 'Fights', 'won': 'Won', 'drawn': 'Drawn', 'lost': 'Lost', 'winrate': 'Winrate', 'boostNumber': 'Boosts used'})
        df_output = df_output[['Rank', 'Wallet', 'Fights', 'Won', 'Drawn', 'Lost', 'Score', 'Winrate', 'Boosts used', 'Reward ($XKI)']]
        ...
        if leaderboardCol.count_documents({'wallet': children}) > 0 :
            rank = leaderboardCol.find_one({'wallet': children}, {'_id': False})["rank"]
            number_players = leaderboardCol.count_documents({})
            rank =  str(int(rank)) + "/" + str(number_players)
            #get reward
            xki_reward = leaderboardCol.find_one({'wallet': children}, {'_id': False})["reward"]
        
        #get xki performance overall
        xki_spent = 0
        if walletDataCol.count_documents({"$and": [{'wallet': children},{'arena': "League 1"}]}) > 0 :
            xki_spent = 10
            xki_spent_wallet = list(walletDataCol.aggregate([{"$match": {'wallet': children}},{"$group": {"_id": "null","totalAmount": {"$sum": "$xkiSpent"}}}]))
        ...
        xki_str = "{:.7}".format(xki_reward - xki_spent) + " xki"
